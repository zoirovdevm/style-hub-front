  ...
  return (
    <aside className="w-full shrink-0 space-y-8 lg:w-64">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold uppercase tracking-wider">{dict.product.filters}</h3>
        <button onClick={() => router.push(pathname)} className="text-xs font-semibold text-ink-900/50 hover:text-ink-950">
      ...
      <div>
        <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-ink-900/50">{dict.nav.categories}</h4>
        <div className="flex flex-col gap-2">
          <button
            onClick={() => updateParams((p) => p.delete('category'))}
            className={`text-left text-sm ${!activeCategory ? 'font-bold text-ink-950' : 'text-ink-900/60 hover:text-ink-950'}`}
          >
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`text-left text-sm ${activeCategory === cat.slug ? 'font-bold text-ink-950' : 'text-ink-900/60 hover:text-ink-950'}`}
            >
      ...
      <div>
        <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-ink-900/50">{dict.product.size}</h4>
            ...
            <button
              ...
              className={`h-9 min-w-9 rounded-lg border px-2 text-xs font-semibold transition-colors ${
                activeSizes.includes(size)
                  ...
                  : 'border-ink-900/15 text-ink-900/70 hover:border-ink-950'
              }`}
            >
              {size}
            </button>
      ...
      <div>
        <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-ink-900/50">{dict.product.color}</h4>
        <div className="flex flex-wrap gap-2">
          {COLORS.map((color) => (
            <button
              key={color.name}
              ...
              className={`h-8 w-8 rounded-full border-2 transition-transform hover:scale-110 ${
                activeColors.includes(color.name) ? 'border-gold-500 ring-2 ring-gold-500/40' : 'border-ink-900/10'
              }`}
            />
          ))}
        </div>
      ...
      <div>
        <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-ink-900/50">{dict.product.priceRange}</h4>
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="0"
            className="w-full rounded-lg border border-ink-900/15 px-3 py-2 text-sm outline-none focus:border-ink-950"
          />
